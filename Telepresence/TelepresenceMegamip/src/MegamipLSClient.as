package  
{
	...
	public static var CONNECTED:String = 'ClientConnected';  // client is connected 
	public static var UPDATE:String='MessageReceived';       // message received    
    ...
    public function onStatusChange(e:StatusChangeEvent):void {
      if (e.status == LSClient.DISCONNECTED) {
         trace("Disconnected from Push Server");
      } else if (e.status == LSClient.CONNECTING) {
        trace("Trying to connect to Push Server...");
      } else if (e.status == LSClient.POLLING) {
         trace( "Connected to Push Server (Smart polling)");
      } else if (e.status == LSClient.STREAMING) {
		  dispatchEvent(new Event(MegamipLSClient.CONNECTED));
         trace("Connected to Push Server (Streaming)");
      } else if (e.status == LSClient.STALLED) {
         trace("Connection to Push Server stalled");
      }
	    trace("onStatusChange status: " + e.status);
    ...
    public function onChange(evt:NonVisualItemUpdateEvent):void {
		...
		this.message = input;
		trace("onCollChange message: " + input);
